...
const assert = require('assert');
    ...
    const { getRuleOptionsSchema, validate } = require("../shared/config-validator");
...
function emitDeprecatedContextMethodWarning(ruleName, methodName) {
  if (!emitDeprecatedContextMethodWarning[`warned-${ruleName}-${methodName}`]) {
    emitDeprecatedContextMethodWarning[`warned-${ruleName}-${methodName}`] =
      true;
    process.emitWarning(
...
class RuleTester {
  ...
  run(ruleName, rule, test) {
    ...
    linter.defineRule(
      ruleName,
      { ...rule, // Create a wrapper rule that freezes the `context` properties.
            create(context) {
    ...
    function runRuleForItem(item) {
      ...
      if (hasOwnProperty(item, 'options')) {
        ...
        if (
          ...
          (!rule.meta ||
            (rule.meta &&
              (typeof rule.meta.schema === 'undefined' ||
