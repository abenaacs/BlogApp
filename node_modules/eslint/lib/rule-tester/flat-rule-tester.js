...
const assert = require('assert');
    ...
    const { getRuleOptionsSchema } = require("../config/flat-config-helpers");
    const { Linter, SourceCodeFixer, interpolate } = require("../linter");
    const CodePath = require("../linter/code-path-analysis/code-path");
...
class FlatRuleTester {
  ...
  static setDefaultConfig(config) {
    if (typeof config !== 'object' || config === null) {
      throw new TypeError(
        'FlatRuleTester.setDefaultConfig: config must be an object',
      );
    }
  ...
  run(ruleName, rule, test) {
    ...
    const baseConfig = [
      { files: ['**'] }, // Make sure the default config matches for all files
      {
        plugins: {
          ...
          'rule-to-test': {
            rules: {
              [ruleName]: { ...rule, // Create a wrapper rule that freezes the `context` properties.
                                create(context) {
    ...
    function runRuleForItem(item) {
      ...
      let code; let filename; let output; let beforeAST; let afterAST;
      ...
      if (typeof item === 'string') {
        ...
        if (itemConfig.languageOptions && itemConfig.languageOptions.parser) {
          const {parser} = itemConfig.languageOptions;
      ...
      // Verify the code.
      const { getComments, applyLanguageOptions, applyInlineConfig, finalize } =
        SourceCode.prototype;
      const originalCurrentSegments = Object.getOwnPropertyDescriptor(
        CodePath.prototype,
        'currentSegments',
      );
      let messages;
        ...
        this.constructor.describe('valid', () => {
          test.valid.forEach((valid) => {
            this.constructor[valid.only ? 'itOnly' : 'it'](
              sanitize(
                typeof valid === 'object' ? valid.name || valid.code : valid,
              ),
              () => {
