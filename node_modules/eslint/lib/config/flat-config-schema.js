...
const globalsSchema = {
  ...
  validate(value) {
    ...
    for (const key of Object.keys(value)) {
      ...
      if (key !== key.trim()) {
        throw new TypeError(
          `Global "${key}" has leading or trailing whitespace.`,
        );
      }

      if (!globalVariablesValues.has(value[key])) {
        throw new TypeError(
          `Key "${key}": Expected "readonly", "writable", or "off".`,
        );
      }
    }
  },
...
const parserSchema = {
  ...
  validate(value) {
    if (
      ...
      throw new TypeError(
        'Expected object with parse() or parseForESLint() method.',
      );
    }
  },
};
...
const sourceTypeSchema = {
  ...
  validate(value) {
    if (
      typeof value !== 'string' ||
      !/^(?:script|module|commonjs)$/u.test(value)
    ) {
