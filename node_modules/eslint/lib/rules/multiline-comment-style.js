  ...
  create(context) {
    ...
    function isStarredBlockComment([firstComment]) {
      ...
      return (
        lines.length > 0 &&
        lines.every((line, i) =>
          (i === 0 || i === lines.length - 1 ? /^\s*$/u : /^\s*\*/u).test(line),
        )
      );
    }
    ...
    function processStarredBlockComment(comment) {
      ...
      return lines.map((line) =>
        line.replace(allLinesHaveLeadingSpace ? /\s*\* ?/u : /\s*\*/u, ''),
      );
    }
        ...
        if (commentGroup.length > 1) {
            ...
            fix(fixer) {
              ...
              return commentLines.some((value) => value.startsWith('/'))
                ...
                : fixer.replaceTextRange(
                    range,
                    convertToStarredBlock(firstComment, commentLines),
                  );
            },
          });
        } else {
          ...
          if (!/^\s*$/u.test(lines[lines.length - 1])) {
            context.report({
              ...
              fix: (fixer) =>
                fixer.replaceTextRange(
                  [firstComment.range[1] - 2, firstComment.range[1]],
                  `\n${expectedLinePrefix}/`,
                ),
            });
          }
                ...
                fix(fixer) {
                  ...
                  if (errorType === 'alignment') {
                    ...
                    const commentTextStartIndex =
                      lineStartIndex + commentTextPrefix.length;
                  ...
                  for (const [idx, line] of lines.entries()) {
                    ...
                    const lineTextToAlignWith =
                      sourceCode.lines[firstComment.loc.start.line - 1 + idx];
    ...
    return {
      Program() {
        return sourceCode
          ...
          .filter(
            (comment) => !astUtils.COMMENTS_IGNORE_PATTERN.test(comment.value),
          )
          .filter((comment) => {
          ...
          .filter(
            (commentGroup) =>
              !(
                ...
                commentGroup[0].loc.start.line === commentGroup[0].loc.end.line
              ),
          )
