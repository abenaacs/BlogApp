...
module.exports = {
  ...
  create(context) {
    ...
    function checkCallNew(node) {
      ...
      node.arguments
        .filter((arg) =>
          hasExcessParensWithPrecedence(arg, PRECEDENCE_OF_ASSIGNMENT_EXPR),
        )
        .forEach(report);
    }
    ...
    return {
      ...
      Property(node) {
        if (node.computed) {
          ...
          if (
            key &&
            hasExcessParensWithPrecedence(key, PRECEDENCE_OF_ASSIGNMENT_EXPR)
          ) {
            report(key);
          }
        }
      ...
      AssignmentPattern(node) {
        ...
        if (
          right &&
          hasExcessParensWithPrecedence(right, PRECEDENCE_OF_ASSIGNMENT_EXPR)
        ) {
          report(right);
        }
      },
