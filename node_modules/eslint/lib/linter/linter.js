...
const {
  Legacy: { ConfigOps, ConfigValidator, environments: BuiltInEnvironments },
  ...
  for (const ruleId of directiveRules) {
    ...
    if (ruleId === null || !!ruleMapper(ruleId)) {
      ...
      result.directiveProblems.push(
        createLintingProblem({ ruleId, loc: commentToken.loc }),
      );
    }
  }
  return result;
...
function getDirectiveComments(sourceCode, ruleMapper, warnInlineConfig) {
  ...
  sourceCode
    ...
    .forEach((comment) => {
      ...
      if (
        directiveText === 'eslint-disable-line' &&
    ...
    if (match[0].endsWith('*/')) {
      retv = Object.assign(
        ...
        commentParser.parseListConfig(
          commentParser.extractDirectiveComment(match[1]).directivePart,
        ),
      );
    }
  }
...
function parse(text, languageOptions, filePath) {
  const textToParse = stripUnicodeBOM(text).replace(
      ...
      Object.assign(contextInfo, {
        [methodName](...args) {
          return this.sourceCode[
            DEPRECATED_SOURCECODE_PASSTHROUGHS[methodName]
          ](...args);
        },
      }),
    {},
  ...
  const sharedTraversalContext = Object.freeze(
    Object.assign(Object.create(BASE_TRAVERSAL_CONTEXT), {
      ...
      markVariableAsUsed: (name) =>
        sourceCode.markVariableAsUsed(name, currentNode),
      parserOptions: {
        ...languageOptions.parserOptions,
      },
...
class Linter {
  ...
  _verifyWithFlatConfigArrayAndProcessor(
    ...
    const messageLists = blocks.map((block, i) => {
      ...
      if (
        configForRecursive &&
        (text !== blockText || path.extname(blockName) !== originalExtname)
      ) {
        debug(
  ...
  _verifyWithFlatConfigArrayAndWithoutProcessors(
      ...
      if (options.warnInlineConfig) {
        ...
        for (const {
          ...
          Object.keys(inlineConfig.rules).forEach((ruleId) => {
            ...
            if (!rule) {
              inlineConfigProblems.push(
                createLintingProblem({ ruleId, loc: node.loc }),
              );
              return;
            }

            try {
              const ruleOptions = Array.isArray(ruleValue)
                ? ruleValue
                : [ruleValue];
    ...
    const commentDirectives =
      options.allowInlineConfig && !options.warnInlineConfig
        ? getDirectiveCommentsForFlatConfig(sourceCode, (ruleId) =>
            getRuleFromConfig(ruleId, config),
          )
  ...
  _verifyWithConfigArray(textOrSourceCode, configArray, options) {
    ...
    const processor =
      config.processor && configArray.pluginProcessors.get(config.processor);
