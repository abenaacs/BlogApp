...
const ConfigCommentParser = require('../linter/config-comment-parser');
...
class SourceCode extends TokenStore {
  ...
  getComments(node) {
    ...
    if (node.type === 'Program') {
       ...
       */
      if (
        ((node.type === 'BlockStatement' || node.type === 'ClassBody') &&
          node.body.length === 0) ||
  ...
  getInlineConfigNodes() {
    ...
    configNodes = this.ast.comments.filter((comment) => {
      ...
      const { directivePart } = commentParser.extractDirectiveComment(
        comment.value,
      );
      ...
      return (
        comment.type !== 'Line' ||
        !!/^eslint-disable-(next-)?line$/u.test(directiveMatch[1])
      );
    });
  ...
  applyInlineConfig() {
    ...
    this.getInlineConfigNodes().forEach((comment) => {
      const { directiveText, directiveValue } =
